...
class FlyweightFactory {
  ...
  getFlyweight(tagName, displayType = 'block', selfClosing = false) {
    const key = `${tagName}-${displayType}-${selfClosing}`;
    if (!this.flyweights[key]) {
      this.flyweights[key] = new LightElementNode(tagName, displayType, selfClosing);
    }
    return this.flyweights[key];
  }
}

module.exports = { LightNode, LightTextNode, LightElementNode, FlyweightFactory };
